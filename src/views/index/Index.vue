          ...
          <article>
            <section v-for="spus in category.spus" :key="spus.id">
              <div class="img_container">
                <img :xsrc="spus.pic_url" src="../../../src/assets/shoploading.png">
              </div>
              <div class="info">
                ...
                <selector @showDot="showDotFun" :name="spus.name" :foodId="spus.skus[0].id"
...
export default {
  ...
  methods: {
    ...
    lazyLoadImg (index) {
      // 如果该分类已经调用过懒加载，就不用执行下面函数了
      if (index > this.categoryPosition.length - 1 || this.hasLoadArr.indexOf(index) !== -1) {
        return
      }
  ...
  created () {
    ...
    // 根据商店id获取店家信息
    // 根据餐馆id获取食物
    getFoods({restaurantId}).then((response) => {
    ...
    this.$store.dispatch('getRestaurant', {restaurantId})
  },
  components: {
    Bottom,
    selector
